...
- Make **Highest generation TPS** the actual fallback everywhere—not only the visible UI—including older saved profiles, direct API calls, Calibration, Engine Shootout, and measurement history. Runtime Manager's static audit is refreshed to oMLX 0.6.4, LM Studio 0.4.23 Build 1, MTPLX 2.10.1, and optional SwiftLM b709; oMLX 0.6.4 is pinned to its official wheel digest and complete source dependency revisions, while every update still requires a matching local measurement before it can become the fastest route.
- Correct local servers that report a normal Chat Completions `stop` after emitting exactly the configured response ceiling. The private session relay now waits only for the bounded terminal usage event, relabels that exact boundary as `length`, and Pi shows one recoverable warning to send `continue` or raise Max response instead of silently ending a reasoning-only turn. Hub Console now also reconciles Pi's authoritative final `message_end` against streamed deltas, restores a missing suffix or final block, and explicitly identifies a settled reasoning-only turn. The relay does not add prompt, reasoning, or response text to persistent state.
